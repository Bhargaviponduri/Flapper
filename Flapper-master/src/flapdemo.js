...
var FlapDemo = function (display_selector) {
    ...
    getMyValues();
    Promise.resolve(getMyValues()).then((value) => {
        for (var i = 0; i < value.length; i++) {
            var $display = $(_this.$displays[i]);
            if (value[i]) {
                $display.val(value[i]).change();
            }
            else {
                $display.val('').change();
            }
...
const getMyValues = async () => {
    var response = await fetch('https://nobleale.github.io/tapbeers.json')
    const data = await response.json();
    var textvalues = data.tastingroom;
    return textvalues;
}